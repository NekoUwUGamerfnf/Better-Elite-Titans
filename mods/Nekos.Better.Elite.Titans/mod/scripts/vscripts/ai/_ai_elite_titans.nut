...
void function EliteTitanExecution_Wait( entity attacker, vector attackerStartingAngles, bool godmode )
{
WaitFrame()
if( godmode == false )
{
  while( attacker.IsInvulnerable() )
  {
   WaitFrame()
   if( !IsValid( attacker ) )
   return
...
void function SetEliteTitanPostSpawn( entity npc )
{
	...
	if ( npc.IsTitan() )
	{
		...
		if( IsValid( soul ) )
		{
			...
			int shieldhealth = 8000
			if ( GetConVarInt( "elite_titan_harder" ) != 0 )
				shieldhealth = 10000
			if ( GetConVarInt( "elite_titan_easier" ) != 0 )
				shieldhealth = 4000
			soul.SetShieldHealthMax( shieldhealth )
	...
	while( true )
	{
		...
		switch ( difficultyLevel )
		{
			...
			case eFDDifficultyLevel.HARD:
				waittime = RandomFloatRange( 20.0, 40.0 )
				break
			case eFDDifficultyLevel.MASTER:
			case eFDDifficultyLevel.INSANE:
				waittime = RandomFloatRange( 40.0, 60.0 )
				break
		}
		if ( isharder == true )
	...
	while( true )
	{
		...
		if( coretime <= Time() )
		{
	        ...
	        if ( GetConVarInt( "elite_titan_easier" ) != 0 )
		        iseasier = true
			if ( isharder == true )
				waittime = RandomFloatRange( 1.0, 20.0 )
			if ( iseasier == true )
				waittime = RandomFloatRange( 40.0, 60.0 )
			if ( iseasier == false && isharder == false )
				waittime = RandomFloatRange( 20.0, 40.0 )
			coretime = Time() + waittime
		}
		else
		{
