...
{
    if( GameRules_GetGameMode() == FD && GetConVarInt( "elite_titan_player_count" ) == 1 )
    PlayerCheckLoop()
}
...
void function PlayerCheckLoop_thread()
{
 while( true )
 {
  ...
  if ( GetConVarInt( "elite_titan_easier" ) == 1 && GetPlayerArray().len() > GetConVarInt( "elite_titan_player_count_easier" ) )
  ...
  if ( GetConVarInt( "elite_titan_harder" ) == 1 && !( GetPlayerArray().len() > GetConVarInt( "elite_titan_player_count_easier" ) ) && GetPlayerArray().len() > GetConVarInt( "elite_titan_player_count_harder" ) )
